...
// const HeroRight = () => {
//   return (
//     <div className="relative flex justify-center items-center bg-gradient-to-br from-blue-50 to-yellow-50">
...
const HeroRight = () => {
  return (
    <div className="relative flex justify-center items-center bg-gradient-to-br from-blue-50 to-blue-100">
      <div className="relative max-w-sm md:max-w-md w-full">
        {/* Soft glow behind the card */}
        <div
          style={{
            background:
              "radial-gradient(ellipse at 60% 40%, #3b82f655 0%, transparent 70%) rounded",
          }}
          className="hidden lg:flex xl:flex absolute -inset-8 rounded-[3rem] pointer-events-none"
        />

        {/* ── MAIN CARD ── */}
        <div className="relative bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-blue-100">
          {/* Top strip – architectural photo placeholder */}
          <div
            className="relative h-44 w-full overflow-hidden"
            style={{
          ...
          >
            ...
            >
              {/* Buildings */}
              <rect x="0" y="60" width="28" height="40" fill="#1e40af" />
              <rect x="6" y="52" width="14" height="8" fill="#1e40af" />
              <rect x="30" y="40" width="36" height="60" fill="#1d4ed8" />
              <rect x="40" y="30" width="16" height="10" fill="#1d4ed8" />
              <rect x="44" y="22" width="6" height="10" fill="#2563eb" />
              {/* Windows on tall building */}
              {[38, 52, 66, 80].map((y) =>
                [33, 42, 52, 58].map((x) => (
                  <rect
              ...
              <rect x="68" y="55" width="22" height="45" fill="#1e40af" />
              <rect x="92" y="35" width="40" height="65" fill="#1e3a8a" />
              <rect x="100" y="20" width="22" height="15" fill="#1e3a8a" />
              <rect x="108" y="12" width="8" height="10" fill="#1e40af" />
              {/* Highlight tower windows */}
              {[40, 54, 68].map((y) =>
                [95, 105, 118, 128].map((x) => (
                  <rect
              ...
              <rect x="134" y="50" width="28" height="50" fill="#1e40af" />
              <rect x="164" y="30" width="50" height="70" fill="#1d4ed8" />
              <rect x="176" y="18" width="26" height="12" fill="#1d4ed8" />
              <rect x="216" y="45" width="32" height="55" fill="#1e40af" />
              <rect x="250" y="25" width="40" height="75" fill="#1e3a8a" />
              {[30, 44, 58, 72, 86].map((y) =>
                [254, 264, 274, 282].map((x) => (
                  <rect
                    key={`${x}-${y}`}
              ...
              <rect x="292" y="55" width="28" height="45" fill="#1d4ed8" />
              {/* Ground */}
              <rect
                x="0"
                y="99"
            ...
            {/* Overlay label */}
            <div className="absolute top-4 left-4 flex items-center gap-2">
              <div className="bg-blue-400 text-white p-2 rounded-xl">
                <Building2 size={16} />
              </div>
              <span className="text-white text-xs font-semibold tracking-widest uppercase opacity-90">
                The Creator's
          ...
          <div className="p-5 space-y-6">
            {/* ── Process strip ── */}
            <div className="rounded-2xl border border-blue-100 p-3.5">
              <div className="flex items-center gap-2 mb-3">
                <Layers size={13} className="text-blue-400" />
                <p className="text-[10px] uppercase tracking-widest font-semibold text-gray-400">
                  How we work
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <span
                    key={s}
                    className="flex items-center gap-1.5 text-[11px] font-medium bg-blue-50 border border-blue-100 text-blue-700 px-2.5 py-1 rounded-full hover:bg-blue-100 hover:border-blue-300 hover:text-blue-800 transition-all cursor-default"
                  >
                    <CheckCircle2 size={10} className="text-blue-400" />
                    {s}
                  </span>
        ...
        <div className="absolute -bottom-3 -right-3 h-10 w-10 rounded-full bg-blue-400 opacity-70 blur-sm pointer-events-none" />
        <div className="absolute -top-3 -left-3 h-6 w-6 rounded-full bg-blue-300 opacity-60 blur-sm pointer-events-none" />
      </div>
    </div>
  );
};
